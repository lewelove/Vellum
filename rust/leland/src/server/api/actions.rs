...
async fn resolve_target_ids(
    ...
    if let Some(q) = query_arg {
        ...
        let root = recursive_arg
            .map_or_else(|| library_root.to_path_buf(), |dir| libleland::utils::expand_path(&dir));
        ...
        let p = libleland::utils::expand_path(&dir).join("album.lock.json");
...
pub async fn execute_action(
    ...
    if let Some(run_str) = &action_cfg.run {
        let expanded_action_path = libleland::utils::expand_path(run_str);
