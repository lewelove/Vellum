...
pub async fn serve_interface_asset(
    ...
    let p = libleland::utils::expand_path(asset_val);
...
pub async fn trigger_full_reset(State(state): State<Arc<AppState>>) -> Response {
    ...
    .await
    .unwrap_or_else(|_| (0, libleland::lua::LogicManifest::default()));
...
async fn evaluate_reload_payloads(
    ...
    tokio::task::spawn_blocking(move || {
        let config_path = libleland::lua::resolve_config_path().unwrap_or_default();
        payloads
            .into_par_iter()
            .map(|item| {
                ...
                let eval_res = libleland::lua::get_or_init_lua_vm(&config_path, |engine| {
...
pub async fn force_update_album(
    ...
    if tokio::fs::try_exists(&path).await.unwrap_or(false) {
        let _ = tokio::process::Command::new("leland")
            .arg("update")
            .arg("--force")
            .arg("--silent")
            .arg(path)
