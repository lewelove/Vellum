...
use libleland::utils::expand_path;
use std::process::Stdio;

pub async fn execute(name: Option<String>) -> Result<()> {
    let raw_name = name.unwrap_or_else(|| "default".to_string());
    let name = raw_name.replace('-', "_");
    let config = libleland::lua::ResolvedConfig::load().context("Failed to load config")?;
    ...
    let dir_str = intf_cfg.directory.unwrap_or_else(|| format!("~/.local/share/leland/interfaces/{raw_name}"));
