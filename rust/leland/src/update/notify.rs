    ...
    tokio::spawn(async move {
...

        {
            ...
            for (album_root, lock_json) in &updated_albums {
                let album_id = libleland::resolvers::rel_path(album_root, &lib_root_for_task);
                    ...
                    .filter_map(Result::ok)
                {
                    let p = entry.path();
                    if p.is_file() {
                        let rel = libleland::resolvers::rel_path(p, &lib_root_for_task);
            ...
            for missing in missing_paths {
                let album_id = libleland::resolvers::rel_path(&missing, &lib_root_for_task);
