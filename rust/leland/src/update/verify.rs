...
pub fn find_missing_paths(
    ...
    let current_album_ids: HashSet<String> = all_albums
        .iter()
        .map(|p| libleland::resolvers::rel_path(p, library_root))
        .collect();
...
fn is_album_dirty(
    ...
    let album_id = libleland::resolvers::rel_path(album_root, library_root);
...
fn deep_verify_dirty(
        ...
        .filter_map(Result::ok)
    {
        let p = entry.path();
        if p.is_file() {
            let rel = libleland::resolvers::rel_path(p, library_root);
