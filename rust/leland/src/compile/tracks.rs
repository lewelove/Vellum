...
use libleland::compiler::manifest::extract_strict_u32;
use libleland::compiler::validation::validate_track_indices;
...
pub fn build_ctx_tracks(
    ...
    album_root: &Path,
    cache_root: &Path,
) -> Result<(Vec<Value>, u64), LelandError> {
    ...
    for (idx, _track_val) in primary_tracks.iter().enumerate() {
        if is_virtual {
            ...
            let harvest = harvest::harvest_file_cached(file_path, cache_root).map_err(|source| LelandError::HarvestError { path: file_path.clone(), source })?;
            let rel_path = libleland::resolvers::rel_path(file_path, album_root);
            let file_info = libleland::utils::get_file_info(file_path, &rel_path, false).unwrap_or_else(|_| json!({}));
...
pub fn build_final_tracks(
    ...
    for (i, t_val) in primary_tracks.iter().enumerate() {
        ...
        let t_title = t_val.get("title").and_then(Value::as_str).ok_or_else(|| LelandError::TypeMismatch {
        ...
        let t_info = json!({
            ...
            "duration_formatted": libleland::resolvers::format_ms(ctx_track.get("duration_milliseconds").and_then(Value::as_u64).unwrap_or(0)),
        });
...
pub fn validate_audio_files(
    ...
    if !is_virtual && audio_files.len() != primary_tracks.len() {
        return Err(LelandError::PhysicalInventoryMismatch {
            path: album_root.to_path_buf(),
