...
use libleland::models::CoverMetrics;
...
pub fn resolve_cover_data(
    ...
    if let Some(cp) = &main_cover_path {
        ...
        if !content.is_empty() {
            cover_hash_address = libleland::utils::calculate_blake3_address(&content);
            let raw = blake3::hash(&content);
            let cover_hash_full = format!("blake3-{}", STANDARD.encode(raw.as_bytes()));
            let rel_path = libleland::resolvers::rel_path(cp, album_root);
            if let Ok(info) = libleland::utils::get_file_info(cp, &rel_path, false) {
...
pub fn resolve_cover_metrics(
    ...
    if let Some(img) = loaded_image {
        if metrics.chroma.is_none() {
            metrics.chroma = Some(libleland::images::cover_chroma::calculate_chroma(img));
            needs_save = true;
        }
        if metrics.entropy.is_none() {
            metrics.entropy = Some(libleland::images::cover_entropy::calculate_entropy(img));
            needs_save = true;
        }
    }
