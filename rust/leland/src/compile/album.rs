...
pub fn parse_mandatory_album_fields(
    primary_album: &Value,
    album_root: &Path,
) -> Result<(String, String, String), LelandError> {
    let get_album_str = |k: &str| -> Result<String, LelandError> {
...
pub fn generate_lock_manifests(
    ...
    for (name, _) in parsed_manifests {
        ...
        if let Ok(info) = libleland::utils::get_file_info(&abs_p, &file_name, false) {
