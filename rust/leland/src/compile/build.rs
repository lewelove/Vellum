...
use libleland::compiler::manifest::load_manifests;
use libleland::error::LelandError;
...
pub fn build(
    album_root: &Path,
    config: &libleland::lua::ResolvedConfig,
) -> Result<Value, LelandError> {
    ...
    let primary_manifest = parsed_manifests.get("metadata").ok_or_else(|| LelandError::MissingPrimaryManifest { path: album_root.to_path_buf() })?;
    let primary_tracks = primary_manifest.get("tracks").and_then(Value::as_array).ok_or_else(|| LelandError::MissingTracksBlock { path: album_root.to_path_buf() })?;
    ...
    let total_discs = libleland::resolvers::calculate_total_discs(primary_tracks);
    let total_tracks = primary_tracks.len() as u32;

    let cache_root = libleland::utils::expand_path(&config.app.storage.cache);
    ...
    let id_str = libleland::resolvers::rel_path(album_root, &library_root);
    ...
    let lua_res = libleland::lua::get_or_init_lua_vm(&config.path, |engine| {
        engine.execute_dispatcher(&ctx, &manifests_json)
    }).map_err(|e| LelandError::ManifestParseError { path: album_root.to_path_buf(), source: toml::de::Error::custom(e.to_string()) })?; 
    ...
    let info_obj = json!({
        ...
        "total_tracks": total_tracks,
        "duration_milliseconds": duration_sum_ms,
        "duration_formatted": libleland::resolvers::format_ms(duration_sum_ms),
    });
