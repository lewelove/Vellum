//! Unit tests for Lua file system utilities (`dl.fs`).
...
fn test_dl_fs_exists() {
    ...
    let code = format!("return dl.fs.exists('{path_str}')");
    let exists: bool = engine.lua.load(&code).eval().expect("Execution failed");
    assert!(exists);

    let not_exists: bool = engine
        .lua
        .load("return dl.fs.exists('/tmp/non_existent_dale_test_file.tmp')")
        .eval()
...
fn test_dl_fs_read_lines() {
    ...
    let code = format!("return dl.fs.read_lines('{path_str}')");
...
fn test_dl_fs_read_json() {
    ...
    let code = format!("local t = dl.fs.read_json('{path_str}')\nreturn t.artist");
    let artist: String = engine.lua.load(&code).eval().expect("Execution failed");
    assert_eq!(artist, "Sample");

    let missing_code = "return dl.fs.read_json('/tmp/non_existent_dale_test_file.tmp')";
    ...
    let bad_code = format!("return dl.fs.read_json('{bad_path}')");
...
artist = "Sample""#;
    ...
    let code = format!("local t = dl.fs.read_toml('{path_str}')\nreturn t.album.artist");
    let artist: String = engine.lua.load(&code).eval().expect("Execution failed");
    assert_eq!(artist, "Sample");

    let missing_code = "return dl.fs.read_toml('/tmp/non_existent_dale_test_file.tmp')";
    let missing_res: Option<Table> = engine.lua.load(missing_code).eval().expect("Execution failed");
    assert!(missing_res.is_none());

    let bad_file = TempFile::new("[album");
    let bad_path = bad_file.path_str();
    let bad_code = format!("return dl.fs.read_toml('{bad_path}')");
    let bad_res = engine.lua.load(&bad_code).eval::<Table>();
    assert!(bad_res.is_err());
}
