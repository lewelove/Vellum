...
fn register_native_functions(lua: &Lua) -> Result<()> {
    ...
    let ll: mlua::Table = globals.get("ll").unwrap_or_else(|_| lua.create_table().unwrap());
    ...
    ll.set("json", json_table).map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    ll.set("toml", toml_table).map_err(|e| anyhow::anyhow!("{e}"))?;

    globals.set("ll", ll).map_err(|e| anyhow::anyhow!("{e}"))?;
    Ok(())
}
...
impl LuaEngine {
    ...
    pub fn evaluate_config(&self, path: &Path) -> Result<EvaluatedLuaData> {
        ...
        let get_manifest: mlua::Function = globals
            .get("__LELAND_GET_LOGIC_MANIFEST")
            .map_err(|e| anyhow::anyhow!("{e}"))?;
    ...
    pub fn evaluate_album_logic(
        ...
        let eval_fn: mlua::Function = globals
            .get("__LELAND_EVALUATE_ALBUM_LOGIC")
            .map_err(|e| anyhow::anyhow!("{e}"))?;
        let opts = SerializeOptions::new()
            .serialize_none_to_null(false)
            .serialize_unit_to_null(false);
...
pub fn resolve_config_path() -> Option<PathBuf> {
    if let Some(home_config) = dirs::home_dir().map(|h| h.join(".config/leland/init.lua"))
        && home_config.exists()
    {
        return Some(home_config);
    }

    if let Ok(env_path) = std::env::var("LELAND_CONFIG_PATH") {
