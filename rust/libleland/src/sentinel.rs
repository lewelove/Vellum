...
pub fn verify_trust(album_root: &Path, expected_id: Option<&str>) -> Result<TrustState, LelandError> {
    ...
    let lock_content = fs::read_to_string(&lock_path)
        .map_err(LelandError::ManifestIoError)?;

    let lock_json: serde_json::Value = serde_json::from_str(&lock_content)
        .map_err(LelandError::JsonError)?;
