...
pub enum LelandError {
    ...
    #[error("Inventory Mismatch in {path}: Found {files_count} audio files on disk but metadata.toml defines {tracks_count} tracks. Leland requires an explicit 1:1 mapping between disk files and metadata entries.")]
