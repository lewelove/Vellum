...
) -> Result<serde_json::Map<String, Value>, LelandError> {
    let metadata_path = album_root.join("metadata.toml");
    if !metadata_path.exists() {
        return Err(LelandError::MissingPrimaryManifest { path: album_root.to_path_buf() });
    }
...
fn parse_single_manifest(
    ...
    let parsed_toml = toml::from_str::<toml::Value>(&content)
        .map_err(|source| LelandError::ManifestParseError { path: path.to_path_buf(), source })?;
    ...
    let tracks_obj = if let Some(tracks_arr) = json_val.get_mut("tracks").and_then(Value::as_array_mut) {
        if tracks_arr.is_empty() && expected_tracks.is_some() {
            ...
            {
                return Err(LelandError::TrackCountMismatch {
...
pub fn extract_strict_u32(val: Option<&Value>, name: &str, default: Option<u32>) -> Result<u32, LelandError> {
    let Some(v) = val else {
        return default.map_or_else(
            || Err(LelandError::InvalidIdentityFormat {
    ...
    match v {
        Value::Number(n) => n
            ...
            .ok_or_else(|| LelandError::InvalidIdentityFormat {
                field: name.to_string(),
                message: "Value exceeds 32-bit integer limits".to_string(),
            }),
        Value::String(s) => {
            let base = s.split('/').next().unwrap_or("").trim();
            base.parse::<u32>().map_err(|_| LelandError::InvalidIdentityFormat {
                field: name.to_string(),
        ...
        _ => Err(LelandError::InvalidIdentityFormat {
            field: name.to_string(),
