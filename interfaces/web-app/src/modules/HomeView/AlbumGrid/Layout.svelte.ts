...
export class LayoutManager {
  ...
  get gapX() {
    return config.album_grid.spacing.x;
  }
  get gapY() {
    return config.album_grid.spacing.y;
  }
  get cardSize() {
    return config.album_grid.album_card.cover.size;
  }

  get creaseHeight() {
    return config.album_grid.spacing.top;
  }
  ...
  get cols() {
    return Math.max(
      1,
      Math.floor((this.containerWidth - 40 + this.gapX) / (this.cardSize + this.gapX))
    );
  }
  get gridWidth() {
    return this.cols * this.cardSize + (this.cols - 1) * this.gapX;
  }
