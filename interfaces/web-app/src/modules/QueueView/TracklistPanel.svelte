    ...
    <div class="tracks-list-container">
      <div class="tracks-list">
        {#each groupedQueue as group, groupIdx (group.albumId || groupIdx)}
          ...
          {@const isMultiDiscAlbum =
            group.albumMeta && Number(group.albumMeta.total_discs || 1) > 1}

          {#each group.tracks as track, i (track.id)}
            {@const showDiscHeader =
              isMultiDiscAlbum && (i === 0 || track.discNo !== group.tracks[i - 1].discNo)}
              ...
              <div class="disc-header-row" class:first-disc={i === 0}>
                ...
                <div class="disc-header-right">
                  <span class="v-mono disc-duration-label"
                    >{getDiscDuration(group.tracks, track.discNo)}</span
                  >
                </div>
              </div>
            {/if}
