...
The `dale x` command is used to run **Dale Actions** for selected albums or the entire library. Each action is called by `dale x action_name`.

## Main Argument

#### `<name>`

Provides match to `[dl.actions].<name>` in `dale.lua`. If `<name>` matches:

- Loads `[dl.config].environment` file
...
- `dale x action_name string --flag` -> will pass `"options": [ "string", "--flag" ]` in intermediary JSON
- `dale x --id "Album/Directory" action_name` -> will consume `--id` as `dale x` option

This way you can write actions that consume any kind of their own flags, options and arguments.

## Built-In Actions

Built-in actions are provided by dale binary out of the box.

- Configure them in `dale.lua` by `dl.actions({ <name> = { config = {} } })`.
- Override them entirely by `dl.actions({ <name> = { run = "" } })` key with your own scripts if you really want.
...
```bash
dale x -f '~/music/Path/To/Album/album.lock.json' action_name
```

```bash
dale x -f '~/music/Path/To/Album/' action_name
```
...
```bash
dale x -p action_name
```

```bash
dale x --playing action_name
```
...
```bash
dale x --id 'Library/Relative/Path/To/Album/Directory' action_name
```
...
```jsonc
{
  ...
  "config": {
    // `dl.config({})` table -> json
    "dale": {},
    // `dl.actions({ <name> = { config = {} } })` table -> json
    "action": {},
  },
...
For action to become executable it must be registered in `dale.lua` config. 
...
dl.actions({
  ...
  action_name = {
    ...
    -- defaults to `~/.local/share/dale/actions/{name}/{name}
    run = "",
