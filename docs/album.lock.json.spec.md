...
{
  // type definitions
  // values below are used to determine type of dale value for each lock file key
  //
  // "" -> string
  // 1 -> number
  // [] -> array of strings
  ...
  "album": {
    ...
    // Album path relative to `dl.config.storage.library`.
    "id": ""

    // User configurable keys from `dl.compile.album.key`.
    "keys": {},
  ...
  "tracks": [
    {
      ...
      // User keys provided by `dl.compile.tracks.key`
      "keys": {},
      "info": {
        "sample_rate": 1,
        "bits_per_sample": 1,
