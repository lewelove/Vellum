...
- Master image is generated using `size`, `filter` and `ratio` and saved as `~/.cache/dale/covers/{cover_name}/master/{address_hash}.qoi`
- If `cache` -> for each `cache[idx]` the cache image is generated via the resize of the master image using `cache[idx].filter` and `cache[idx].size` and saved as `~/.cache/dale/covers/{cover_name}/static/{filter}/{size}/{address_hash}.qoi`
