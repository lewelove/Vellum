...
```lua
-- "album" -> "a"
dl.compile.a.key({ key_name = function(ctx, m) return "I am truncated!" end })
```
...
```lua
-- "tracks" -> "track" -> "t"
dl.compile.track.key({ key_name = function(ctx, m, i) return "hi" end  })
dl.compile.t.key({ key_name = function(ctx, m, i) return "thank you for reading docs..." end })
```
...
dl.compile.album.key({
  musicbrainz_release_url = function(ctx, manifests)
    ...
    dl.fn.type_check(id, "string") -- `album.musicbrainz_releaseid` in `id.toml` must be a string
...
```lua
dl.compile.album.key({ key_name_1 = function(ctx, m) return "Hello, World!" end })
dl.compile.tracks.key({ key_name_2 = function(ctx, m, i) return 5318008 end })
```

Invalid syntax. Each `dl.compile.{level}.key({})` must contain single element only.
